...
It is heavily recommended to use a computer using 64-bit Ubuntu 22.04 for this project. 

### 1. VexRiscv

VexRiscv (Scala + sbt + ...)
RISCV-GNU-Toolchain (and exported to path, in opt/)

```sh
# navigate to the root folder of VexRiscv repo copy
cd ./quartus/stock/vexriscv/VexRiscv-master

# compile selected core, under quartus/stock/vexriscv/VexRiscv-master/src/main/scala/vexriscv/demo/GenAvalonFullNoMmuMaxPerf.scala
sbt "runMain vexriscv.demo.GenAvalonFullNoMmuMaxPerf"
```

`VexRiscvAvalonMaxPerf_hw.tcl` and `VexRiscvAvalonMaxPerf.v` should be generated and appear in the VexRiscv root folder. The `.tcl` file is the Qsys-wrapped component to use in Qsys. The `.v` file is the verilog RTL description of the core. It must have the following lines manually added to the very top of the file.
```
`define SYNTHESIS
```

Copy both of these (modified) files into `quartus/stock/vexriscv/cores/VexRiscvAvalonMaxPerf`, the folder that Quartus points to for components.

### 2. Quartus

Open the project `stock-vexriscv.qpf` in `quartus/stock/vexriscv` with Quartus Prime (Lite v21).

Tools->Platform Designer and load the `vexriscv_system.qsys` Qsys system. The updated component definition will be detected automatically. `Generate HDL..` with the default settings and save.

Run `Compilation and Synthesis` using the default top level file `DE1_SoC_top_level`.

Launch Tools->Programmer and select the `output_files/*.sof` file generated by synthesis and compilation. This is likely selected by default. Ensure you have selected programming the FPGA __only__, as in the image.

![Quartus Programmer Setup](./doc/images/Quartus_Programmer.png)

The FPGA LED will blink if the image has been loaded successfully.

#### Setup for Programming the FPGA Board

On Ubuntu the USB Blaster for Quartus Programmer is not natively supported. To recognise the USB Blaster for first time setup we must add a device management rule.

Check if the USB Blaster is recognised:
  1. Plug the USB Blaster into the computer and a powered-on board.
  2. Navigate from the Quartus install location to `./intelFPGA_lite/21.1/quartus/bin`.
  3. Restart the jtag daemon with `sudo killall -9 jtagd` then `sudo ./jtagd`.
  4. Run `sudo ./jtagconfig`.
      a. Note that ‘sudo’ is required to access all ports, especially if Quartus was not installed on the user account.
  5. If this returns “No JTAG hardware available” then you must follow the steps below to configure your Ubuntu environment.
  6. If this returns “1) DE-SoC […] …” then the board is being recognised and the JTAG has been configured correctly.

Following the forum post here to configure Ubuntu device manager:
  1. Navigate to the device management directory `cd /etc/udev/rules.d/`.
  2. Create a new file ‘altera-usb-blaster.rules’ in this directory, with the below contents.
  3. Create a new file ‘51-usbblaster.rules’ in this directory, with the below contents.
  4. Copy the file “pgm_parts.txt” from the Quartus installation (e.g., from ‘./intelFPGA_lite/21.1/quartus/linux64/’) into the `./intelFPGA_lite/21.1/quartus/bin` directory (i.e., the same folder as jtagd).
  5. Repeat the steps from above to check if the USB Blaster is recognised. No restart should be required. This is a one-time setup.
      a. Note: this solution is bodged together and many of these steps are likely redundant. RocketBoards suggest that only the ‘51-usbblaster.rules’ file is required. 
...
Navigate to `quartus/stock/vexriscv/sw`, the software folder. Navigate to the `testing` folder for the test project to test all instructions, with debugging flags enabled.

```sh
# from within 'quartus/stock/vexriscv/sw/testing'
make

# the below should be visible:
# /opt/riscv/bin/riscv64-unknown-elf-objdump -D out/bootrom.elf > out/bootrom.dump
# /opt/riscv/bin/riscv64-unknown-elf-objcopy -O binary out/bootrom.elf out/bootrom.bin
```
The elf file is located in `./out`, and will be used to program the core.

To run OpenOCD and gdbgui to begin debugging, step out with `cd ..` to return to `quartus/stock/vexriscv/sw`, and run:
```sh
./debug.sh
```
...
openocd -f interface/ftdi/c232hm.cfg \
    ...
    -f vexriscv_init.cfg
```
To open gdbgui and begin debugging:
```sh
...
1. vexriscv generation
2. quartus Qsys and component
3. compile and synthesise Quartus
4. Compile software with makefile
5. openOCD and gnu gdbgui
...
Current custom IP search path is: `P4P-41/quartus/stock/vexriscv/cores/**/*"`
